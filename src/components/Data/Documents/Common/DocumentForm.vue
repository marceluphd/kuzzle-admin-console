<template>
  <form class="DocumentForm card-panel wrapper "  @submit.prevent="create">
    ...
    <!-- Json view -->
    <div class="DocumentForm-contentEditors row json-view" v-if="!isFormView">
      ...
      <div class="col s6 card" v-if="!$store.state.collection.isRealtimeOnly">
        <div class="card-content">
          ...
          <pre v-json-formatter="{content: $store.getters.simplifiedMapping, open: true}"></pre>
        </div>
      </div>
    </div>
